...
async def content_chunk_by_llm(content:str)->list[str]:
    completion = await asyncAzureOpenAIStructedOutputClient.beta.chat.completions.parse(
    model=os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME","gpt-4o"), # replace with the model deployment name of your gpt-4o 2024-08-06 deployment
    messages=[
...
LLM_CHUNK_PATH = os.getenv("LLM_CHUNK_PATH","./llmChunk/") 
...
async def process_big_chunk_file(file_path):
    ...
    async with SEM:  # semaphore to control the number of concurrent requests
        ...
        if chunkResult:
            path, name, ext = parse_file_path(file_path)
